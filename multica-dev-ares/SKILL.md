...
15. **项目绑定必须向下贯穿到所有子工单**（老板 2026-07-09 固化）：开绑定了 project 的工单（`multica issue create --project <PID>`）时，**由 leader / 执行 agent 派生出来的子工单（子任务）也必须绑定同一个 project**，不能只绑父单、漏了子单。做法两条同时上：① PM 拆单时在给 leader 的执行指令里明确写「本单及其所有派生子工单都绑 project <PID>」；② PM 发现有子单漏绑（`multica issue get <子单> --output json` 看 `project_id` 为空），立即 `multica issue update <子单> --project <PID>` 补绑。目的：project 视图能收全整条线，不漏子单。
